...
class Ticket(BaseModel):
    ...
    priority: Optional[TicketPriority] = Field(
        TicketPriority.MEDIUM, description="Urgency level"
    )
    ...
    # AI-generated fields
    ai_suggested_category: Optional[TicketCategory] = Field(
        None, description="AI-predicted category"
    )
    ai_suggested_priority: Optional[TicketPriority] = Field(
        None, description="AI-predicted priority"
    )
    ai_suggested_reply: Optional[str] = Field(
        None, description="AI-generated draft response"
    )
    ai_confidence: Optional[float] = Field(
        None, description="AI confidence score (0-1)"
    )

    # AI analysis fields
    ai_analysis: Optional[Dict[str, Any]] = Field(
        None, description="Complete AI analysis result"
    )
    sentiment: Optional[str] = Field(
        None, description="Customer sentiment: positive, neutral, or negative"
    )
    urgency_keywords: list[str] = Field(
        default_factory=list, description="Detected urgency indicators"
    )
    extracted_metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="AI-extracted entities (order_id, amounts, dates)",
    )

    # Metadata
    created_at: datetime = Field(
        default_factory=datetime.utcnow, description="Ticket creation timestamp"
    )
    ...
    order_id: Optional[str] = Field(
        None, description="Related order number (if applicable)"
    )
