...
try:
    ...
    # TensorFlow 2.15 uses standalone keras package
    ...
    print(f"⚠️  TensorFlow not available. Install with: pip install tensorflow")
    print(f"    Error: {e}")
...
class TicketForecastingService:
    ...
    def __init__(self, model_path: str = "./models/lstm_forecast.h5"):
        ...
        if not TENSORFLOW_AVAILABLE:
            print("⚠️  Forecasting service disabled - TensorFlow not available")
            self.model = None
            self.scaler = None
            return
    ...
    def prepare_sequences(
        self, data: np.ndarray, sequence_length: int = 24
    ...
    def predict_next_hours(
        self, recent_data: List[int], hours: int = 24
    ) -> List[Dict[str, Any]]:
        """
    ...
    def predict_daily_volume(
        self, recent_data: List[int], days: int = 7
    ) -> List[Dict[str, Any]]:
        """
        ...
        for i, pred in enumerate(hourly_predictions):
            ...
            if (i + 1) % 24 == 0:
                daily_predictions.append(
                    {
                        "date": (
                            current_date + timedelta(days=len(daily_predictions) + 1)
                        ).isoformat(),
